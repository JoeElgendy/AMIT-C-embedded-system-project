...
bool SDB_ReadEntry(uint8 id,uint8* year , uint8* subjects, uint8* grades){
    int place=search((int) id);
    if (place != -1){
        year=&db[place].Student_Year;
        subjects=&(db[place].CoursesAndGrades.courses_id[0]);     // the module user will access the rest of grades by using *(subject+1)   
                                                                // i considered using self referential structure with multiple links but i guess it wouldn't differ that match

        grades=&(db[place].CoursesAndGrades.courses_grades[0]);  
        return true;
    }
     else {
        return false;
